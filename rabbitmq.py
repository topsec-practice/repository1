...
def setup_rabbitmq_connection():
    ...
    return pika.BlockingConnection(pika.ConnectionParameters('47.108.169.120', 5672, '/', user_info))
...
def main():
    while True:
        try:
            ...
            def get_next_file_id(user_id):
                cursor.execute("SELECT MAX(file_id) FROM files WHERE user_id = %s", (user_id,))
                max_id = cursor.fetchone()[0]
                if max_id is None:
                    return 0  # 该用户没有记录，从0开始
                else:
                    # 假设file_id是整数类型
                    return int(max_id) + 1
...
def insert_file_and_matches(data, cursor):
    ...
    if "policy_id" in data and "rule_id" in data:
        insert_match_record(data,cursor)
    
    print(f" [√] 插入/更新文件记录: {data['file_id']}, count={count}")

def update_file_and_matches(data, cursor):
    """Update file record and associated match records"""
    # Delete old match records
    cursor.execute("DELETE FROM matches WHERE user_id = %s AND file_id = %s", 
                  (data["user_id"], data["file_id"]))
    
    # Update file record
    insert_file_and_matches(data, cursor)

def insert_or_update_file_and_matches(data, cursor):
    """Insert or update file record and match records"""
